from keras.layers import Input, Dense, Conv2D, ConvLSTM2D, MaxPooling2D, UpSampling2D, Flatten, Reshape, Conv2DTranspose, Lambda, Dropout, TimeDistributed
...
from loading import get_image_blocks_lstm
...
class MonitorCallback(Callback):
    def __init__(self, monitor='loss', value=0.01, verbose=0, model=None, y_true=None):
        ...
        self.model = model
        self.y_true = y_true
        self.y_true_squeezed = np.squeeze(y_true)

    def on_epoch_end(self, epoch, logs={}):
        current = logs.get(self.monitor)
        y_pred = self.model.predict(self.y_true, verbose=0)
        y_pred = np.squeeze(y_pred)
        mssim_c = MultiScaleSSIM(y_pred*255.0, self.y_true_squeezed*255.0)
...
input_img = Input(shape=(1, height, width, 3), name="encoder_input")

# Full autoencoder
# Encoding part
x = ConvLSTM2D(filters=64, kernel_size=(3, 3), strides=(2,2), activation='relu', name="encoder_2", return_sequences=True, input_shape=(1, height, width, 3))(input_img)
x = ConvLSTM2D(filters=128, kernel_size=(3, 3), strides=(2,2), activation='relu', name="encoder_3", return_sequences=True)(x)
x = ConvLSTM2D(filters=128, kernel_size=(3, 3), strides=(2,2), activation='relu', name="encoder_4", return_sequences=True)(x)
x = TimeDistributed(Flatten(), name="encoder_5")(x)
...
x_train, size = get_image_blocks_lstm(file_name=image_name+".tiff", h=height, w=width)
...
decoded_imgs = decoder.predict(np.squeeze(codes))
